...
if (isloggedin()){ // take the user to the section in the course id, accroding to the parameters sent from the refferer
	...
	if (strpos($_SERVER['HTTP_REFERER'],'mapa-linux-test') > -1){
		redirect('https://mapa-linux-test.ort.org.il/madait-login/login.html#courseid='.$courseid.'&amp;section='.$sectionid);// Lea 2015/08 - &amp; needed in order to prevent &sect from becoming ยง
	}
	else{
		redirect('https://bigbrain.ort.org.il/login.html#courseid='.$courseid.'&amp;section='.$sectionid);// Lea 2015/08 - &amp; needed in order to prevent &sect from becoming §
	}
}
