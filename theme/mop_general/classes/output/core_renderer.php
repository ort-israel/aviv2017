...
namespace theme_mop_general\output;
...
class core_renderer extends \theme_fordson\output\core_renderer {
    ...
    public function standard_head_html() {
        ...
        $output .= "<link href=\"https://fonts.googleapis.com/css?family=Heebo:200,400,700,800&amp;subset=hebrew\" rel=\"stylesheet\">\n";
        return $output;
    }
    ...
    public function context_header($headerinfo = null, $headinglevel = 1) {
        // For teachers' mooc -
        // The rest of the pages should display the name and short name of the course
        $course = ($this->page->context->contextlevel == CONTEXT_COURSE ||
                    $this->page->context->contextlevel == CONTEXT_MODULE) ? $this->page->course : null;
        if ($course) {
            $ret = html_writer::start_tag('h1');
            $ret .= html_writer::tag('sapn', $course->fullname,array('class' => 'fullname'));
            $ret .= html_writer::tag('sapn', $course->shortname,array('class' => 'shortname'));
            $ret .= html_writer::end_tag('h1');
            return $ret;
        }
    ...
    public function teacherdash() {
        ...
        $summary = theme_mop_general_strip_html_tags($COURSE->summary);
        $summarytrim = theme_mop_general_course_trim_char($summary, 300);
        $courseinfo = array(
            array(
        ...
        return $this->render_from_template('theme_mop_general/teacherdash', $dashlinks);
    ...
    //Tsofiya: Many of these function may be used in the new theme, if not needed in new theme - delete

    /**
     * Put logo in top menu. Called from header.mustache
     * Lea 2017/7. Code taken from earlier function context_header.
     * In order for it to return the file, a condition is added to function theme_mop_general_pluginfile in theme/mop_general/lib/filesettings_lib.php
     */
    ...
    public function footer_about() {
        $theme = theme_config::load('mop_general');
        $setting = $theme->settings->footnote_about;
        return $setting != '' ? $setting : '';
    }

    public function footer_details_title() {
        return get_string('contactandsupporttitle', 'theme_mop_general');
    }
