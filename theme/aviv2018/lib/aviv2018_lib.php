...
function theme_aviv2018_get_course_activities() {
 ...
 * @return string
 */
function theme_aviv2018_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
function aviv2018_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $PAGE, $CFG;
        ...
        if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
            ...
            foreach ($mycourseschildrennodeskeys as $k) {
                ...
                if ($CFG->navshowmycoursecategories) {
                    ...
                    $allchildrennodes = aviv2018_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 */
function aviv2018_boostnavigation_find_privatefiles_node(global_navigation $navigation) {
 ...
 * @return array
 */
function aviv2018_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    if (count($navigationnode->children) == 0) {
        ...
        foreach ($childrennodeskeys as $ck) {
            $allchildren = array_merge($allchildren, aviv2018_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
        }
 ...
 * @return void
 */
function aviv2018_navigation_custom_menu_item(custom_menu_item $menunode, $parent, $pmasternode) {
    global $PAGE, $CFG;
    ...
    if ($menunode->has_children()) {
        ...
        if ($parent > 0) {
            $masternode = $pmasternode->add(aviv2018_local_navigation_get_string($menunode->get_text()), $url, navigation_node::TYPE_CONTAINER);
 ...
 * @return string
 */
function aviv2018_local_navigation_get_string($string) {
    $title = $string;
