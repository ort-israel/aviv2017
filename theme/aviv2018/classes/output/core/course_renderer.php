...
//        $content .= html_writer::start_tag('div'/*, array('class' => 'info')*/);
        ...
        if (!@getimagesize($featuredimage)) { // check taken from here: https://stackoverflow.com/a/29716622/278
            ...
            if (!empty($categoryidnum)) {
                ...
                if (file_exists($headerbgfile)) {
                    $headerbg = new moodle_url($CFG->wwwroot . '/theme/aviv2018/pix/cat_' . $categoryidnum . '.jpg');
                }
            }
            if (empty($headerbg)) {
    ...
    public function search_courses($searchcriteria) {
        ...
        if (!empty($searchcriteria)) {
            ...
            $courses = \theme_aviv2018\output\core\coursecat::search_courses($searchcriteria, $chelper->get_courses_display_options());
