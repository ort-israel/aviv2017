...
function theme_aviv2020_get_course_activities() {
...
function theme_aviv2020_strip_html_tags($text) {
    $text = preg_replace(
        array(
 ...
 * @return string
 */
function theme_aviv2020_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
function aviv2020_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $PAGE, $CFG;
...
//    if (!empty($PAGE->theme->settings->removeprivatefilesnode)) {
    // If yes, do it.
    if ($privatefilesnode = aviv2020_boostnavigation_find_privatefiles_node($navigation)) {
        ...
        if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
            ...
            foreach ($mycourseschildrennodeskeys as $k) {
                ...
                if ($CFG->navshowmycoursecategories) {
                    ...
                    $allchildrennodes = aviv2020_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 */
function aviv2020_boostnavigation_find_privatefiles_node(global_navigation $navigation) {
 ...
 * @return array
 */
function aviv2020_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    if (count($navigationnode->children) == 0) {
        ...
        foreach ($childrennodeskeys as $ck) {
            $allchildren = array_merge($allchildren, aviv2020_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
        }
 ...
 * @return void
 */
function aviv2020_local_navigation_extend_navigation(global_navigation $navigation) {
    global $PAGE;
    ...
    if ($menu->has_children()) {
        foreach ($menu->get_children() as $item) {
            aviv2020_navigation_custom_menu_item($item, 0, null);
        }
    }
}
 ...
 * @return void
 */
function aviv2020_navigation_custom_menu_item(custom_menu_item $menunode, $parent, $pmasternode) {
    global $PAGE, $CFG;
    ...
    if ($menunode->has_children()) {
        ...
        if ($parent > 0) {
            $masternode = $pmasternode->add(aviv2020_local_navigation_get_string($menunode->get_text()), $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
        } else {
            $masternode = $PAGE->navigation->add(aviv2020_local_navigation_get_string($menunode->get_text()), $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
            $masternode->showinflatnavigation = true;
        }
        foreach ($menunode->get_children() as $menunode) {
            aviv2020_navigation_custom_menu_item($menunode, $submenucount, $masternode);
        }
    } else {
        $url = $CFG->wwwroot;
        if ($menunode->get_url() !== null) {
 ...
 * @return string
 */
function aviv2020_local_navigation_get_string($string) {
    $title = $string;
