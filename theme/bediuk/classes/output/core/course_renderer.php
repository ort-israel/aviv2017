...
namespace theme_bediuk\output\core;
...
//        $content .= html_writer::start_tag('div'/*, array('class' => 'info')*/);
        ...
        if (!@getimagesize($featuredimage)) { // check taken from here: https://stackoverflow.com/a/29716622/278
            ...
            if (!empty($categoryidnum)) {
                ...
                if (file_exists($headerbgfile)) {
                    $headerbg = new moodle_url($CFG->wwwroot . '/theme/bediuk/pix/cat_' . $categoryidnum . '.jpg');
                }
            }
            if (empty($headerbg)) {
                /* If category doesn't have an idnumber, or there isn't an image to correspond the that number, insert default image */
                $headerbg = new moodle_url($CFG->wwwroot . '/theme/bediuk/pix/category_default.jpg');
            }
    ...
    public function search_courses($searchcriteria) {
        ...
        if (!empty($searchcriteria)) {
            ...
            $courses = \theme_bediuk\output\core\coursecat::search_courses($searchcriteria, $chelper->get_courses_display_options());
