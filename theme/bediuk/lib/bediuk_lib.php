...
function theme_bediuk_get_course_activities() {
...
function theme_bediuk_strip_html_tags($text) {
    $text = preg_replace(
        array(
 ...
 * @return string
 */
function theme_bediuk_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
function bediuk_boostnavigation_extend_navigation(global_navigation $navigation) {
    global $PAGE, $CFG;
...
//    if (!empty($PAGE->theme->settings->removeprivatefilesnode)) {
    // If yes, do it.
    if ($privatefilesnode = bediuk_boostnavigation_find_privatefiles_node($navigation)) {
        ...
        if ($mycoursesnode = $navigation->find('mycourses', global_navigation::TYPE_ROOTNODE)) {
            ...
            foreach ($mycourseschildrennodeskeys as $k) {
                ...
                if ($CFG->navshowmycoursecategories) {
                    ...
                    $allchildrennodes = bediuk_boostnavigation_get_all_childrenkeys($mycoursesnode->get($k));
 ...
 */
function bediuk_boostnavigation_find_privatefiles_node(global_navigation $navigation) {
 ...
 * @return array
 */
function bediuk_boostnavigation_get_all_childrenkeys(navigation_node $navigationnode) {
    ...
    if (count($navigationnode->children) == 0) {
        ...
        foreach ($childrennodeskeys as $ck) {
            $allchildren = array_merge($allchildren, bediuk_boostnavigation_get_all_childrenkeys($navigationnode->get($ck)));
        }
 ...
 * @return void
 */
function bediuk_local_navigation_extend_navigation(global_navigation $navigation) {
    global $PAGE;
    ...
    if ($menu->has_children()) {
        foreach ($menu->get_children() as $item) {
            bediuk_navigation_custom_menu_item($item, 0, null);
        }
    }
}
 ...
 * @return void
 */
function bediuk_navigation_custom_menu_item(custom_menu_item $menunode, $parent, $pmasternode) {
    global $PAGE, $CFG;
    ...
    if ($menunode->has_children()) {
        ...
        if ($parent > 0) {
            $masternode = $pmasternode->add(bediuk_local_navigation_get_string($menunode->get_text()), $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
        } else {
            $masternode = $PAGE->navigation->add(bediuk_local_navigation_get_string($menunode->get_text()), $url, navigation_node::TYPE_CONTAINER);
            $masternode->title($menunode->get_title());
            $masternode->showinflatnavigation = true;
        }
        foreach ($menunode->get_children() as $menunode) {
            bediuk_navigation_custom_menu_item($menunode, $submenucount, $masternode);
        }
    } else {
        $url = $CFG->wwwroot;
        if ($menunode->get_url() !== null) {
 ...
 * @return string
 */
function bediuk_local_navigation_get_string($string) {
    $title = $string;
